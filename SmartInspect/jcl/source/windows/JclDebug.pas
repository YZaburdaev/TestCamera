...
begin
  ...
  begin
    ...
    {$IF DEFINED (SiExceptFMX) or DEFINED(SiExceptVCL)}
    try
      if (P^.ExceptObject <> nil) then
        SiExcept.LogFatal(Format('Получен стек Delphi Exception Address [%p] %s',[P^.ExceptAddr, TObject(P^.ExceptObject).ClassName]))
      else
        SiExcept.LogFatal(Format('Получен стек Delphi Exception Address [%p] nil',[P^.ExceptAddr]));
    except
      // заглушить на всякий случай чтоб не получилась рекурсия исключений
    end;
    {$ENDIF}
  ...
  begin
    ...
    {$IF DEFINED (SiExceptFMX) or DEFINED(SiExceptVCL)}
    try
       SiExcept.LogFatal(Format('Получен стек Exception Address [%p]', [P^.ExceptionAddress]));
    except
      // заглушить на всякий случай чтоб не получилась рекурсия исключений
    end;
    {$ENDIF}
